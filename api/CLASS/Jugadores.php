...
class Jugadores
{
    ...
    public function getJugadoresByTeam(){
        ...
        try {
            $SQL="SELECT * FROM jugadores WHERE equipo = :id;";
            $gsent = $this->CONN->prepare($SQL);
            $gsent->bindValue(':id', $this->equipo, PDO::PARAM_INT);
        ...
        try {
            ...
            if($gsent->execute()){
                ...
                if($cuenta>0){
                    $data = array(
                        'code' =>'0',
                        'description' => "Jugador actualizado con exito!",
                        'player' => [
                            'id' => $this->id,
                            'equipo' => $this->equipo,
                            'nombre' => $this->name,
